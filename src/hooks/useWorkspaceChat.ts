...
export const useWorkspaceChat = (workspaceId: string | undefined) => {
  ...
  const loadMessages = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await chatService.getWorkspaceMessages(workspaceId);
      setMessages(data);
    } catch (err: unknown) {
  ...
  // Server-side filter is workspace-wide (NULL project filtering is not
  // supported by postgres_changes filters), so project-scoped rows are
  // excluded defensively in the client as well.
  useEffect(() => {
    if (!workspaceId || !supabaseConfig.isConfigured) return;
      ...
      .on(
        ...
        async (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            // Defensive scope check: this channel only serves global messages
            if (newRow.project_id !== null) return;
            ...
            if (updatedRow.project_id !== null) return;
  ...
  const sendMessage = async (content: string): Promise<void> => {
    if (!workspaceId) throw new Error('Workspace is required to send a message.');
    ...
    try {
      const persisted = await chatService.sendWorkspaceMessage(workspaceId, user.id, {
        content: trimmed,
      });
